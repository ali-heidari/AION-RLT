# AION-RLT Standards

This project follows the `ai-agent-standards` conventions.

## Documentation requirements

- The repository includes a `docs/` folder at the root.
- `docs/index.md` lists the available documentation files.
- All documentation is written in Markdown.

## AI agent instructions

The local AI instruction file is located at `.github/copilot-instructions.md`.
The canonical standard base is maintained in `https://github.com/ali-heidari/ai-agent-standards`.
